#!/usr/bin/env python3
"""
Interactive session for inspecting a summarization adapter.

Loads a base model, optionally with a LoRA adapter on top, and opens a small
REPL: pick a test paper, view the exact prompt and its tokens, generate a
summary with the adapter on or off, compare both against the abstract and the
teacher summary, and check that the adapter's tokenizer and chat template match
the base model's. The prompt is the one used at training time.

Usage:
    # 8B adapter (bf16)
    python evaluation/interactive_inference.py \
    ...
    # 70B adapter, base model loaded in 4-bit
    python evaluation/interactive_inference.py \
        --base_model meta-llama/Llama-3.3-70B-Instruct \
        --adapter_path results/llama3.3-70b-qlora/train/adapter --use_qlora ...

    # Base model only
    python evaluation/interactive_inference.py --base_model meta-llama/Llama-3.1-8B-Instruct

--dataset, --summaries and --test_ids are optional; without them there is no
paper list to pick from. Type "help" at the prompt for the commands.

The adapter directories under results/ do not contain the weight file; point
--adapter_path at a directory that has adapter_model.safetensors.
"""
...
class InteractiveSession:
    ...
    def cmd_tokens(self):
        ...
        # The chat template already emits BOS, so the prompt must be encoded
        # with add_special_tokens=False. Encoding with the default is shown
        # alongside to make the duplicated BOS visible.
        ...
        if len(old_token_ids) != len(token_ids):
            ...
            print(f"    First 3 with default encoding: {old_token_ids[:3]}")
            print(f"    First 3 as used:               {token_ids[:3]}")
            print()

        print_header(f"PROMPT TOKENS ({len(token_ids)} total)")
        show_n = 30

#!/usr/bin/env python3
"""
Evaluation pipeline, variant with prompt and truncation aligned to training.

Identical to evaluate.py except for two things:

  - The system message and instruction are the ones used at training time
    (training/finetune_v2.py), instead of the evaluation prompt in evaluate.py.
  - Papers that do not fit the context window are cut with the same
    head-and-tail, section-aware truncation as in training
    (smart_truncate_paper, copied from finetune_v2.py) instead of being cut
    from the front only.

This variant was written after the reported runs and was not used for the
numbers in the paper; it is kept for follow-up experiments. Usage, options and
output files are exactly as for evaluate.py. If smart_truncate_paper changes
in finetune_v2.py, update the copy below.
"""
...
# Prompt used at training time (training/finetune_v2.py); must stay in sync.
SYSTEM_MESSAGE = (
...
# ---------------------------------------------------------------------------
# Truncation, copied from training/finetune_v2.py. Keep in sync.
# ---------------------------------------------------------------------------

#!/usr/bin/env python3
"""
LoRA / QLoRA fine-tuning of Llama 3 on teacher summaries (version 2).

This is the script used for the Llama 3.3 70B run reported in the paper
(4-bit NF4 QLoRA on two 48 GB GPUs; see training/run_finetune_70b.slurm).
The 8B run used finetune_v1.py.

Changes relative to finetune_v1.py:
  - Token-budgeted truncation. For every example the exact number of tokens
    left for the paper text is measured (after the system prompt, instruction,
    title, chat-template tokens and the teacher summary). Papers that do not
    fit keep roughly the first 70% and the last 30% of that budget, cut at
    section or paragraph boundaries where possible (smart_truncate_paper).
    Nothing is dropped for length.
  - Defaults tuned for the 70B run: learning rate 1e-4, LoRA rank 16 /
    alpha 32, dropout 0.1, warmup 5%, max sequence length 8192.
  - A checkpoint per epoch, with the best validation-loss checkpoint restored
    at the end (load_best_model_at_end).
  - Optional early stopping (--early_stopping_patience).
  - The collator masks padding positions in the labels using the attention
    mask (needed if the pad token falls back to EOS) and counts examples
    whose response template was not found.
  - --max_memory_per_gpu to balance the device map across GPUs.
  - training_config.json is written next to the adapter.
  - The inference helper no longer adds a second BOS token.

Pipeline, outputs and the remaining options are as in finetune_v1.py.

Usage (the reported 70B configuration):
    python training/finetune_v2.py \
        --dataset data/dataset.jsonl --summaries data/summaries.jsonl \
        --model meta-llama/Llama-3.3-70B-Instruct --output runs/llama3.3-70b-qlora \
        --per_device_batch_size 1 --gradient_accumulation_steps 16 \
        --lora_rank 16 --lora_alpha 32 --learning_rate 1e-4 --num_epochs 3 \
        --max_seq_length 8192 --max_memory_per_gpu 45GiB --warmup_ratio 0.05

evaluation/evaluate_train_aligned.py carries a copy of smart_truncate_paper()
so that inference can apply the same truncation as training. Keep the two
copies in sync.

Dependencies: see setup/requirements-train.txt
"""
...
# Truncation marker. evaluation/evaluate_train_aligned.py uses the same one.
TRUNCATION_MARKER = "\n\n[...TRUNCATED...]\n\n"
...
def smart_truncate_paper(
    ...
    Inference should apply the same truncation the model was trained with;
    evaluation/evaluate_train_aligned.py carries a copy of this function.

#!/usr/bin/env python3
"""
LoRA / QLoRA fine-tuning of Llama 3 on teacher summaries (version 1).

This is the script used for the Llama 3.1 8B run reported in the paper
(bf16 LoRA, no quantization; see training/run_finetune_8b.slurm). The 70B run
used finetune_v2.py, which differs mainly in how over-length papers are cut
down; its docstring lists the changes.

Pipeline:
  1. Join data/dataset.jsonl (paper text) with data/summaries.jsonl (teacher
     summaries from Claude Opus 4.6) into instruction/response pairs.
  2. Format each pair with the model's chat template. Paper text is cut at
     --max_input_chars characters; examples that still exceed --max_seq_length
     tokens are dropped.
  3. Stratified train/val/test split by domain (cs / physics / math), seeded.
  4. Fine-tune with TRL's SFTTrainer and a completion-only collator, so the
     loss is computed on the summary tokens only.
  5. Save the adapter, the split metadata and the trainer metrics.

Usage:
    # 8B, bf16 LoRA (the reported configuration)
    python training/finetune_v1.py \
        --dataset data/dataset.jsonl --summaries data/summaries.jsonl \
        --model meta-llama/Llama-3.1-8B-Instruct --output runs/llama3.1-8b-lora \
        --no_quantize --per_device_batch_size 4 --gradient_accumulation_steps 4 \
        --lora_rank 16 --lora_alpha 32 --max_seq_length 8192

    # Same with 4-bit NF4 quantization (QLoRA): drop --no_quantize.
    # Resume an interrupted run: add --resume_from_checkpoint.
    # Merge the adapter into the base model after training: --merge_and_save
    # (loads the full model in bf16, ~140 GB for 70B).

Outputs, in --output:
    adapter/              LoRA weights, adapter_config.json, tokenizer files
    test_paper_ids.json   held-out test papers with domain labels (read by evaluation/)
    split_summary.json    per-domain counts for each split
    train_results.json, eval_results.json, all_results.json, trainer_state.json

A single-paper inference mode exists (--inference with --adapter_path and
--paper_file); evaluation/interactive_inference.py is the more convenient tool.
...
def train(
    ...
    # Validate that a checkpoint exists before attempting to resume

#!/usr/bin/env python3
"""
Extract per-summary extractive density and LongDocFACTScore values, joined
on paper_id, for the scatter-plot analysis in Section 4.3.

Outputs one CSV row per (system, paper_id) pair with columns:
  system, paper_id, domain, extractive_density, summary_length_words, longdocfactscore

Run once per model scale; tag the scale via --scale and concatenate the CSVs
afterward.

Usage (70B):
  python extract_per_summary.py \
      --generations /scratch/jam5cq/Summary_Fine_Tuning/Results/Output_70b_qlora/eval_generations.json \
      --scale 70B \
      --output_dir /scratch/jam5cq/Summary_Fine_Tuning/Data_Collection

Usage (8B): same command but with the 8B generations path and --scale 8B.
"""
...
# ===================================================================== #
#                       DATA LOADING (matches evaluate.py)              #
# ===================================================================== #
...
def main():
    ...
    parser.add_argument("--test_ids",
                        default="/scratch/jam5cq/Summary_Fine_Tuning/Output/"
                                "Llama3.1_8b_lora/test_paper_ids.json")
    parser.add_argument("--output_dir",
                        default="/scratch/jam5cq/Summary_Fine_Tuning/Data_Collection")
